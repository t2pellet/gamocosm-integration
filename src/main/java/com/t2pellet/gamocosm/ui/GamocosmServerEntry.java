...
public class GamocosmServerEntry extends MultiplayerServerListWidget.Entry {
    private static final Text TITLE_TEXT = new TranslatableText("gamocosm.server.title");
    ...
    public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
        ...
        this.client.textRenderer.draw(matrices, this.server.getAddress(), (float)(x + 32 + 3), (float)(y + 12), 8421504);
    }
    ...
    public Text getNarration() {
        return new TranslatableText("narrator.select", (new LiteralText("")).append(TITLE_TEXT).append(" ").append(this.server.getName()));
    }
}
